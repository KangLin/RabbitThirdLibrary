...
if [ ! -d ${RABBIT_BUILD_SOURCE_CODE} ]; then
    VERSION=15570e07946c799f17a3c32fe5001bf9e1cd80bf
...
echo "cmake .. -DCMAKE_INSTALL_PREFIX=$RABBIT_BUILD_PREFIX -DZLIB_ROOT=\"$RABBIT_BUILD_PREFIX\" -G\"${GENERATORS}\" ${CMAKE_PARA}"
...
case ${BUILD_TARGERT} in
    ...
    windows_mingw)
        cd $RABBIT_BUILD_PREFIX/lib
        if [ -f minizip.lib ]; then
            cp minizip.lib libminizip.lib
        fi
        ;;
    *)
    ;;
