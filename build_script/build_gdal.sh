...
if [ ! -d ${RABBIT_BUILD_SOURCE_CODE} ]; then
    ...
    if [ "TRUE" = "${RABBIT_USE_REPOSITORIES}" ]; then
        echo "git clone -q --branch=tags/${GDAL_VERSION} https://github.com/OSGeo/gdal ${RABBIT_BUILD_SOURCE_CODE}"
        git clone -q --branch=tags/$GDAL_VERSION https://github.com/OSGeo/gdal ${RABBIT_BUILD_SOURCE_CODE}
    else
...
case ${RABBIT_BUILD_TARGERT} in
    android)
        ...
        LIBS="-lstdc++"
        CONFIG_PARA="CXX=${RABBIT_BUILD_CROSS_PREFIX}g++ LD=${RABBIT_BUILD_CROSS_PREFIX}ld"
