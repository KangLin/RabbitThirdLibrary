...
if (${env:ANDROID_NATIVE_API_LEVEL})
{
    $RABBIT_JOB_NAME = $RABBIT_JOB_NAME + ", ANDROID_NATIVE_API_LEVEL=${env:ANDROID_NATIVE_API_LEVEL}"
} else {
    ${env:ANDROID_NATIVE_API_LEVEL}=18
}
...
{
    $RABBIT_JOB_NAME = $RABBIT_JOB_NAME+ ", QT_VERSION=${env:QT_VERSION}"
    $RABBIT_JOB_NAME = $RABBIT_JOB_NAME+ ", QT_VERSION_DIR=${QT_VERSION_DIR}"
...
$url = "https://ci.appveyor.com/api/buildjobs/$jobToWaitId/artifacts/RABBIT_${env:BUILD_TARGERT}${env:ANDROID_NATIVE_API_LEVEL}_${env:RABBIT_ARCH}_qt${JOB_QT_VERSION}_${env:RABBIT_CONFIG}_${env:BUILD_VERSION}.zip"
echo $url
