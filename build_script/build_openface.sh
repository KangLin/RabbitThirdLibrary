...
if [ -z "$RABBIT_BUILD_SOURCE_CODE" ]; then
    RABBIT_BUILD_SOURCE_CODE=${RABBIT_BUILD_PREFIX}/../src/OpenFace
fi

CUR_DIR=`pwd`
#下载源码:
if [ ! -d ${RABBIT_BUILD_SOURCE_CODE} ]; then
    VERSION=OpenFace_2.2.0
    if [ "TRUE" = "${RABBIT_USE_REPOSITORIES}" ]; then
        echo "git clone -q https://github.com/TadasBaltrusaitis/OpenFace.git ${RABBIT_BUILD_SOURCE_CODE}"
        git clone -q https://github.com/TadasBaltrusaitis/OpenFace.git ${RABBIT_BUILD_SOURCE_CODE}
        if [ "$VERSION" != "master" ]; then
            git checkout -b $VERSION $VERSION
        fi
    else
        echo "wget -q -c https://github.com/TadasBaltrusaitis/OpenFace/archive/${VERSION}.zip"
        mkdir -p ${RABBIT_BUILD_SOURCE_CODE}
        cd ${RABBIT_BUILD_SOURCE_CODE}
        wget -q -c https://github.com/TadasBaltrusaitis/OpenFace/archive/${VERSION}.zip
        unzip -q ${VERSION}.zip
        mv OpenFace-${VERSION} ..
        rm -fr *
        cd ..
        rm -fr ${RABBIT_BUILD_SOURCE_CODE}
        mv -f OpenFace-${VERSION} ${RABBIT_BUILD_SOURCE_CODE}
    fi
fi
