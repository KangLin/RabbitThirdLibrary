...
if [ -z "$RABBIT_BUILD_SOURCE_CODE" ]; then
    RABBIT_BUILD_SOURCE_CODE=${RABBIT_BUILD_PREFIX}/../src/Seeta2
fi
...
if [ ! -d ${RABBIT_BUILD_SOURCE_CODE} ]; then
    ...
    if [ "TRUE" = "${RABBIT_USE_REPOSITORIES}" ]; then
        echo "git clone -q https://github.com/KangLin/SeetaFace2.git ${RABBIT_BUILD_SOURCE_CODE}"
        git clone -q https://github.com/KangLin/SeetaFace2.git ${RABBIT_BUILD_SOURCE_CODE}
        if [ "$VERSION" != "master" ]; then
            git checkout -b v$VERSION v$VERSION
        fi
    else
        echo "wget -q -c https://github.com/KangLin/SeetaFace2/archive/${VERSION}.zip"
        mkdir -p ${RABBIT_BUILD_SOURCE_CODE}
        cd ${RABBIT_BUILD_SOURCE_CODE}
        wget -q -c https://github.com/KangLin/SeetaFace2/archive/${VERSION}.zip
        unzip -q ${VERSION}.zip
        mv SeetaFace2-${VERSION} ..
        rm -fr *
        cd ..
        rm -fr ${RABBIT_BUILD_SOURCE_CODE}
        mv -f SeetaFace2-${VERSION} ${RABBIT_BUILD_SOURCE_CODE}
    fi
fi
